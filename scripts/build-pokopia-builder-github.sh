...
DISPLAY_NAME="Pokopia Builder GitHub"
BUNDLE_IDENTIFIER="${BUNDLE_IDENTIFIER:-com.appleforever11.pokopiabuilder.github}"
SIGNING_IDENTITY="${SIGNING_IDENTITY:-F025E64E839EE98CCB4208CC695132BBBE0CCA6D}"
DIST_DIR="$ROOT_DIR/dist"
BUILD_ROOT="${BUILD_ROOT:-$(mktemp -d /tmp/pokopia-builder-github-dist.XXXXXX)}"
APP_DIR="$BUILD_ROOT/$DISPLAY_NAME.app"
DMG_STAGING="$BUILD_ROOT/dmg-staging-pokopia-builder-github"
DMG_MOUNT="$BUILD_ROOT/dmg-mount-pokopia-builder-github"
DMG_RW="$BUILD_ROOT/Pokopia-Builder-GitHub-arm64-rw.dmg"
DMG_PATH="$BUILD_ROOT/Pokopia-Builder-GitHub-arm64.dmg"
FINAL_APP_DIR="$DIST_DIR/$DISPLAY_NAME.app"
FINAL_DMG_PATH="$DIST_DIR/Pokopia-Builder-GitHub-arm64.dmg"
...
cleanup_build_root() {
  if [[ "${KEEP_BUILD_ROOT:-0}" != "1" && "$BUILD_ROOT" == /tmp/pokopia-builder-github-dist.* ]]; then
    rm -rf "$BUILD_ROOT"
  fi
}
...
swift build -c release --arch arm64 -Xswiftc -DPOKOPIA_GITHUB --product "$APP_NAME"
...
/usr/libexec/PlistBuddy -c "Set :CFBundleDisplayName $DISPLAY_NAME" "$CONTENTS_DIR/Info.plist" 2>/dev/null || \
